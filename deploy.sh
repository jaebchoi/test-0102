...
startup() {
  ...
  helm upgrade --install $INFRA_NAME test-0102-infrastructure \
     --values test-0102-infrastructure/values.yaml \
     --values test-0102-infrastructure/values-dev.yaml
  if ! kubectl rollout status --namespace argocd deployment/argocd-server --timeout=30s; then
    exit $?
  fi
  argocd repo add https://github.com/jaebchoi/test-0102 --server localhost:30080 --plaintext --insecure-skip-server-verification
}
...
deploy() {
  ...
  if is_app_running; then
    echo "test-0102 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying test-0102 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace test-0102 \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/jaebchoi/test-0102 \
      --path test-0102-deploy/src/main/resources \
      --revision $branch \
...
down() {
  if is_app_running; then
    ...
    echo "test-0102 is not deployed"
  fi
}
